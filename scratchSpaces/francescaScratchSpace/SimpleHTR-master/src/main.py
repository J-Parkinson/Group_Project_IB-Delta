...
def infer1(model, fnImg):
    "recognize text in image provided by file path"
    for imgName in os.listdir(fnImg):
        if imgName.endswith(".png"):
            print()
            # print(imgName)
            img = preprocess(cv2.imread(fnImg + imgName, cv2.IMREAD_GRAYSCALE), Model.imgSize)
            # print(img)
            batch = Batch(None, [img])
            (recognized, probability) = model.inferBatch(batch, True)
            print('Recognized:', '"' + recognized[0] + '"')
...
# def inferList(model, listOfImageLocations):
...
def main():
    ...
    if args.train or args.validate:
        ...
        model = Model(open(FilePaths.fnCharList).read(), DecoderType.BestPath, mustRestore=True, dump=None) # change dump to
        # arg.dump if needed
