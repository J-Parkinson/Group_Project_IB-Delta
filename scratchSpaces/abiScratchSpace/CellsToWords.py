...
#################################################################
imgList = []
paths = ["../../imagePreprocessing/segmentedcells/cell62.png"]
...
def cellToWords(cellOfWords, width):
    ...
    newCell = stripCell(cell)
    cv2.imwrite('../../imagePreprocessing/tests/strippedCell.png', newCell)
    rows = cellToRows(newCell, width)
    for rowArr in rows:
        cv2.imwrite('../../imagePreprocessing/tests/row.png', rowArr)
        ...
        for word in words:
            ...
            if word.shape[0]>0 and word.shape[1]>0:
                cv2.imwrite('../../imagePreprocessing/tests/word.png', word)
                newWord = removeWhiteSpaceFromWord(word)
                cv2.imwrite('../../imagePreprocessing/tests/word-removed.png', newWord)
